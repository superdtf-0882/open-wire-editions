...
const { runDeterministic, gA1_3 } = require('./analysis-gates');
...
function runAnalysisGates(edition, cfg, sourceText, opts = {}) {
  ...
  let selfContained = 0, notSelfContained = 0;
  ...
  for (const it of items) {
    ...
    const r = runDeterministic(w, it, {
      ...
      bounds: (cfg.gates && cfg.gates.whyWords)
        ? { minWords: cfg.gates.whyWords.min, maxWords: cfg.gates.whyWords.max, maxSentences: cfg.gates.whyWords.maxSentences }
        : undefined,
      sourceText: sourceText ? sourceText.get(it.id) : '',
    });
    for (const n of r.unverifiedNouns || []) nounObservations[n] = (nounObservations[n] || 0) + 1;
    // SELF-CONTAINMENT, a signal rather than a gate. DTOG 40- section 3:
    // running w against b ALONE asks a different question from running it
    // against b plus source -- not "is this grounded?" but "can a reader who
    // sees only the brief follow it?". Option B answers the first and would
    // SILENCE the second, because every token in the article passes whether
    // or not the reader ever saw it. Kept because discarding a measurement
    // nobody decided to discard is the larger change. GATES NOTHING.
    const sc = gA1_3(w, it, '');
    if (sc.status === 'pass' && !(sc.nounObservation || []).length) selfContained++; else notSelfContained++;
    if (!r.ok) {
  ...
  return {
    ...
    selfContainment: { selfContained, notSelfContained, note: 'w against the brief ALONE -- readability signal, not a gate (DTOG 40- section 3)' },
